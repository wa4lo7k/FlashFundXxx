  ...
  async getAllOrders(): Promise<{ data: any[] | null; error: any }> {
    try {
      ...
      const dataPromise = supabaseAdmin
        ...
        `)
        .order('created_at', { ascending: false })
        // Removed limit to get ALL orders
...
// ============================================================================
// ADMIN NOWPAYMENTS SERVICE
// ============================================================================

export const adminNowPaymentsService = {
  // Get all NowPayments orders
  async getNowPaymentsOrders(): Promise<{ data: any[] | null; error: any }> {
    try {
      console.log('📊 Admin NowPayments: Fetching orders...')
      ...
      console.log('📊 Admin NowPayments: Result:', {
        count: data?.length || 0,
        error: error
      })
      ...
      console.error('❌ Admin NowPayments: Error:', error)
      return { data: null, error }
    }
  },

  // Get NowPayments order by ID
  async getNowPaymentsOrder(orderId: string): Promise<{ data: any | null; error: any }> {
    try {
      const { data, error } = await supabaseAdmin
        .from('orders')
        .select(`
          *,
          user_profiles!inner(email, first_name, last_name, phone)
        `)
        .eq('order_id', orderId)
        .eq('payment_provider', 'nowpayments')
        .single()
  ...
  // Get NowPayments statistics
  async getNowPaymentsStats(): Promise<{ data: any | null; error: any }> {
    try {
      const { data, error } = await supabaseAdmin
        .from('orders')
      ...
      const stats = {
        ...
        total_amount: data.reduce((sum, order) => sum + (order.final_amount || 0), 0),
        paid_orders: data.filter(order => order.payment_status === 'paid').length,
        pending_orders: data.filter(order => order.payment_status === 'pending').length,
        failed_orders: data.filter(order => order.payment_status === 'failed').length,
        success_rate: data.length > 0 ? (data.filter(order => order.payment_status === 'paid').length / data.length * 100).toFixed(1) : '0'
      }
...
export const adminStatsService = {
  ...
  async getDashboardStats(): Promise<{ data: any; error: any }> {
    try {
      ...
      // First, let's get a simple count to see what's happening
      console.log('🔍 Admin Stats: Getting simple counts...')
      const { count: ordersCount, error: ordersCountError } = await supabaseAdmin
        .from('orders')
        .select('*', { count: 'exact', head: true })

      console.log('🔍 Admin Stats: Orders count result:', { ordersCount, ordersCountError })
      ...
      console.log('🔍 Admin Stats: Cache bust timestamp:', timestamp)

      const dataPromise = Promise.all([
        // Get ALL orders for accurate count
        supabaseAdmin.from('orders').select('order_id, order_status, payment_status, final_amount, amount, created_at, user_id').order('created_at', { ascending: false }),
        // Get ALL users for accurate count
        supabaseAdmin.from('user_profiles').select('id, first_name, last_name, email, created_at').order('created_at', { ascending: false }),
        // Get ALL delivered accounts for accurate count
        supabaseAdmin.from('delivered_accounts').select('id, user_id, account_type, delivered_at, order_id, current_phase, account_size').order('delivered_at', { ascending: false })
      ])
      ...
      console.log('📊 Admin Stats: Processed data:', {
        ...
        sampleOrderIds: orders.slice(0, 5).map((o: any) => o.order_id),
        orderStatuses: orders.map((o: any) => o.order_status)
      })

      // Calculate statistics with safe field access
      const stats = {
        totalOrders: ordersCount || orders.length, // Use the count if available, fallback to array length
        ...
        completedOrders: orders.filter((order: any) => order.order_status === 'completed').length,
