...
export default function AdminDashboard() {
    ...
    try {
      ...
      if (statsError) {
        ...
        console.log('✅ Admin Dashboard: Stats loaded successfully:', {
          ...
          recentOrdersCount: statsData?.recentOrders?.length,
  ...
  // Use stats data instead of limited orders array for accurate counts
  ...
  // Calculate additional stats from recent orders (for display purposes only)
  ...
  const getRecentActivity = () => {
    ...
    orders.slice(0, 10).forEach((order, index) => {
      ...
      activities.push({
        id: `order-${order.order_id}-${index}`, // Make unique key
    ...
    deliveredAccounts.slice(0, 5).forEach((account, index) => {
      const user = users.find(u => u.id === account.user_id)
      const timeAgo = new Date(account.delivered_at).toLocaleString()

      activities.push({
        id: `account-${account.id}-${index}`, // Make unique key using account.id
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-400">
                        {activity.orderId} • {activity.user}
